...
// for convenience
#define CreateHashMap(hashM, ktype, vtype, capacity) \
  HashMap<ktype, vtype, capacity> hashM
#define CreateComplexHashMap(hashM, ktype, vtype, capacity, comparator) \
  HashMap<ktype, vtype, capacity> hashM(comparator)
...
class HashMap {
 public:
  ...
  || #
  ||
  || @parameter compare optional function for comparing a key against another
  (for complex types)
  */
  ...
  */
  unsigned int size() const { return currentIndex; }
  ...
  */
  K keyAt(unsigned int idx) { return keys[idx]; }
  ...
  */
  V valueAt(unsigned int idx) { return values[idx]; }
  ...
  */
  bool willOverflow() { return (currentIndex + 1 > capacity); }
  ...
  */
  ...
  void setNullValue(V nullv) { nil = nullv; }
