...
// static uint8_t lastKeyData[5] = {0, 0, 0, 0, 0};

void analogControl() {
  keyboard.GetValue();
  /*
    if (lastKeyData != keyboard.keyData) {*/
  esp_now_send(espnow.peer_addr, keyboard.keyData, 3); /*
 }

 memcpy(lastKeyData, keyboard.keyData, 5 * sizeof(uint8_t));*/
}
...
void displayCarInfo() {
  ...
  M5.Lcd.setCursor(110, 20, 2);
  M5.Lcd.print("N : ");
  M5.Lcd.print(getDistance(269));
  M5.Lcd.print("     ");

  M5.Lcd.setCursor(220, 20, 2);
  M5.Lcd.print("W : ");
  M5.Lcd.print(getDistance(269 + 90));
  M5.Lcd.print("     ");
}
