...
void Espnow::BotInit(void) {
  ...
  // sscanf(WiFi.softAPmacAddress().c_str(), "%x:%x:%x:%x:%x:%x",  &sta_addr[0],
  // &sta_addr[1], &sta_addr[2], &sta_addr[3], &sta_addr[4], &sta_addr[5] );
  // WiFi.softAPmacAddress()
  // if(!bot)
  sscanf(WiFi.macAddress().c_str(), "%x:%x:%x:%x:%x:%x", &sta_addr[0],
         &sta_addr[1], &sta_addr[2], &sta_addr[3], &sta_addr[4], &sta_addr[5]);
  // else
  // sscanf(WiFi.softAPmacAddress().c_str(), "%x:%x:%x:%x:%x:%x",  &sta_addr[0],
  // &sta_addr[1], &sta_addr[2], &sta_addr[3], &sta_addr[4], &sta_addr[5] );
  ...
  sscanf(mac_addr.c_str(), "%x:%x:%x:%x:%x:%x%c", &peer_addr[0], &peer_addr[1],
...
void Espnow::OnRemotSent(const uint8_t *mac_addr,
                         esp_now_send_status_t status) {}
int8_t Espnow::OnRemotRecv(const uint8_t *mac_addr, const uint8_t *data,
                           int data_len) {
  if (data_len == 4) {
    connectflag = true;
  }
...
int8_t Espnow::OnBotRecv(const uint8_t *mac_addr, const uint8_t *data,
                         int data_len) {
  ...
  if (updatetime >= 300) {
    ...
    while (true) {
      ...
      for (int i = 0; i < connect_num; i++) {
        M5.Lcd.setCursor(
            30, 25 + 25 * i);  // Set cursor near top left corner of screen
        M5.Lcd.printf("%02x:%02x:%02x:%02x:%02x:%02x", connect_addr[i][0],
                      connect_addr[i][1], connect_addr[i][2],
                      connect_addr[i][3], connect_addr[i][4],
                      connect_addr[i][5]);
      }
      if (connect_num) {
      ...
      if (ypos < 1) {
        // M5.Lcd.fillScreen(TFT_BLACK);
        if (old_ypos != ypos) M5.Lcd.fillRect(30, 210, 90, 50, TFT_BLACK);
        M5.Lcd.setCursor(30, 210);
        M5.Lcd.println("update");
      } else {
        if (old_ypos != ypos) M5.Lcd.fillRect(30, 210, 100, 50, TFT_BLACK);
        // M5.Lcd.fillScreen(TFT_BLACK);
      ...
      if ((digitalRead(37) == LOW) && (connect_num)) {
        ...
        // Serial.printf("%2x:%2x:%2x:%2x:%2x:%2x",slave.peer_addr[0],slave.peer_addr[1],slave.peer_addr[2],
        ...
        while (!connectflag) {
          // for(int i = 0;i < 6; i++){
          esp_err_t result =
              esp_now_send(slave.peer_addr, sta_addr, sizeof(sta_addr) + 1);
...
void Espnow::sendData(void) {
  ...
  for (int i = 0; i < SlaveCnt; i++) {
    ...
    // WiFi.softAPmacAddress()
    // esp_err_t result = esp_now_send(scan_addr, WiFi.softAPmacAddress(),
    // sizeof(WiFi.softAPmacAddress()));
...
void Espnow::BotConnectUpdate(void) {
  ...
  if (updatetime >= 100) {
    ...
    while (true) {
      ...
      for (int i = 0; i < connect_num; i++) {
        M5.Lcd.setCursor(
            30, 75 + 25 * i);  // Set cursor near top left corner of screen
        M5.Lcd.printf("%02x:%02x:%02x:%02x:%02x:%02x", connect_addr[i][0],
                      connect_addr[i][1], connect_addr[i][2],
                      connect_addr[i][3], connect_addr[i][4],
                      connect_addr[i][5]);
